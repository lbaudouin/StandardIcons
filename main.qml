...
Window {
    ...
    Component.onCompleted: {
        var arrayOfLines = icons.match(/[^\r\n]+/g);
        for( var i=0; i<arrayOfLines.length; i++){
            var str = arrayOfLines[i]
            var words = str.split('\t');
            model.append( {
                             name: words[0],
                             description: words[1]
    ...
    property string icons: "address-book-new	The icon used for the action to create a new address book.
